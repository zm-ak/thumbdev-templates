...
| `claude-resume` ラベル | 必須 | 途中で失敗したIssue実装を再開する用。未作成なら `gh label create claude-resume` で作成可 |
...
| claude-issue-to-pr.yml | タスク（Issue）1件 | `feature/issue-<n>-*` ブランチの有無と、Issue上の「## 作業経過」コメント | Issueに `claude-resume` ラベルを付与する、または `@claude` とコメントする |
...
- どこまで終わっていて次に何をすべきかを、完了マーカーファイル（docs pipeline）または
  Issueへの「## 作業経過」コメント（issue-to-pr / plan-to-code）として残す。
  これにより、再開時のClaudeは経過を読むだけで済み、状況の再分析からやり直す必要がない。
